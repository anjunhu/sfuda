...
def eval_and_label_dataset(dataloader, model, banks, args):
    ...
    if args.distributed:
        ...
        all_sensitives = concat_all_gather(all_sensitives.cuda())
        ...
        all_sensitives = remove_wrap_arounds(all_sensitives, ranks)

    assert len(logits) == len(dataloader.dataset)
    pred_labels = logits.argmax(dim=1)
    probs = F.softmax(logits, dim=1).detach()
    accuracy = (pred_labels == gt_labels).float().mean()
    auc = calculate_auc(probs[:, 1].cpu(), gt_labels.cpu())
    ...
    for sa in range(args.data.sens_classes):
        gt_labels_sa = gt_labels[all_sensitives==sa]
        all_preds_sa = pred_labels[all_sensitives==sa]
        all_logits_sa = logits[all_sensitives==sa]
        all_probs_sa = probs[all_sensitives==sa]
        wandb_dict[f"Test Acc A{sa}"] = (all_preds_sa == gt_labels_sa).float().mean()
        wandb_dict[f"Test AUC A{sa}"] = calculate_auc(all_probs_sa[:, 1].cpu(), gt_labels_sa.cpu())
...
def train_target_domain(args):
    ...
    if args.learn.queue_size == -1:
        ...
        dummy_dataset = ImageList_idx(label_file, args.data.image_root)
...
def train_epoch(train_loader, model, banks, optimizer, epoch, args):
    ...
    top1_ins = AverageMeter("SSL-Acc@1", ":6.4f")
    top1_psd = AverageMeter("CLS-Acc@1", ":6.4f")
