...
def data_load(args):
    ...
    if args.test_resampling:
        test_weights = dsets["source_te"].get_weights(args.test_resampling, flag=args.flag)
        test_sampler = WeightedRandomSampler(test_weights, len(test_weights), replacement=True, generator=g)

    dset_loaders["source_tr"] = DataLoader(dsets["source_tr"], batch_size=train_bs, sampler=train_sampler, num_workers=args.worker, drop_last=False)
    dset_loaders["source_te"] = DataLoader(dsets["source_te"], batch_size=train_bs, sampler=test_sampler, num_workers=args.worker, drop_last=False)
...
def cal_acc(loader, netF, netB, netC, flag=False):
    ...
    mean_ent = torch.mean(Entropy(all_output.softmax(1))).cpu().data.item()
    auc = calculate_auc(all_output.softmax(1)[:, 1], all_label.cpu())
    ...
    group_metrics[f'auc'] = auc
    group_metrics[f'acc'] = accuracy
    group_metrics[f'ent'] = mean_ent
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='ckps/AaD/source')
    parser.add_argument('--da', type=str, default='uda')
    parser.add_argument('--trte', type=str, default='val', choices=['full', 'val'])

    parser.add_argument('--wandb', action='store_true', help="Use wandb")
    parser.add_argument('--run_name', type=str, default='VISDA-C_source')
    ...
    run_name = ''.join(['AaD_Source_', args.dset, '_', args.flag, names[args.s][0].upper()])
    if args.wandb:
