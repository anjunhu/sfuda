...
def train_source_domain(args):
    ...
    if args.data.dataset == "imagenet-1k":
        ...
        train_dataset = ImageList(
            label_file, args.data.image_root, transform=train_transform
        )
        val_dataset = ImageList(
            label_file, args.data.image_root, transform=val_transform
        )
...
def train_epoch(train_loader, model, optimizer, epoch, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        sensitives = data[2].cuda(args.gpu, non_blocking=True)
...
def evaluate(val_loader, model, domain, args, wandb_commit=True):
    ...
    gt_labels, all_logits, all_sensitives = [], [], []
    with torch.no_grad():
    ...
    if args.distributed:
        ...
        all_sensitives = remove_wrap_arounds(all_sensitives, ranks).cpu()

    all_preds = all_logits.argmax(dim=1)
    accuracy = (all_preds == gt_labels).float().mean()
    all_probs = F.softmax(all_logits, dim=1).detach().cpu()
    auc = calculate_auc(all_probs[:, 1], gt_labels.cpu())
    wandb_dict = {f"{domain} Acc": accuracy, f"{domain} AUC": auc,}

    for sa in range(args.data.sens_classes):
        gt_labels_sa = gt_labels[all_sensitives==sa]
        all_preds_sa = all_preds[all_sensitives==sa]
        all_logits_sa = all_logits[all_sensitives==sa]
        all_probs_sa = all_probs[all_sensitives==sa]
        wandb_dict[f"{domain} Acc A{sa}"] = (all_preds_sa == gt_labels_sa).float().mean()
